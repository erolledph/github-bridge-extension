...
class ExtensionApp {
    ...
    try {
      ...
      for (const [relativePath, zipEntry] of Object.entries(zipContent.files)) {
        if (!zipEntry.dir) {
          ...
          // Always extract as Uint8Array for consistent byte-level representation
      ...
      if (!existingFiles.has(path)) {
        ...
        try {
          ...
          if (existingContentResponse.success) {
            // Get existing content from GitHub (already UTF-8 decoded)
            ...
            try {
              ...
              // If UTF-8 decoding fails, this might be a binary file
              // Compare as binary by converting both to base64
              console.log(`Binary file detected: ${path}, comparing as base64`);
              
              // Convert uploaded content to base64
              const uploadedBase64 = this.arrayBufferToBase64(uploadedFile.content);
              
              // Get existing content as base64
              ...
              if (existingBase64Response.success) {
                ...
                // If we can't get base64 content, assume modified
                changes.modified.push({ path, file: uploadedFile, status: 'modified' });
              }
              continue;
            }
            
            // Normalize line endings and whitespace for text files
            existingContent = this.normalizeContent(existingContent);
            uploadedContent = this.normalizeContent(uploadedContent);
            
            // Debug logging for comparison (can be removed in production)
            if (existingContent !== uploadedContent) {
              console.log(`File differs: ${path}`);
              console.log(`Existing length: ${existingContent.length}, Uploaded length: ${uploadedContent.length}`);
              
              // Show first difference for debugging
              for (let i = 0; i < Math.min(existingContent.length, uploadedContent.length); i++) {
                if (existingContent[i] !== uploadedContent[i]) {
                  console.log(`First difference at position ${i}: existing="${existingContent.charCodeAt(i)}" vs uploaded="${uploadedContent.charCodeAt(i)}"`);
                  break;
                }
              }
            }
            ...
            if (existingContent === uploadedContent) {
              ...
              console.log(`✗ File modified: ${path} (content differs)`);
              changes.modified.push({ path, file: uploadedFile, status: 'modified' });
            }
          } else {
  ...
  normalizeContent(content) {
    return content
      ...
      .replace(/\r\n/g, '\n')
      // Remove carriage returns that might remain
      .replace(/\r/g, '')
      // Remove trailing whitespace from each line
      .replace(/[ \t]+$/gm, '')
      // Remove multiple consecutive newlines
      .replace(/\n{3,}/g, '\n\n')
      // Ensure file ends with single newline if it has content, otherwise empty
      .replace(/\n*$/, content.trim() ? '\n' : '');
  }
